...
header.header_distance=Inches(0)
...
if sidesert:
   with st.expander('Select Steps for Sidesert Process',expanded=True):
      ...
      # Main Process Name
      SecMainStepNum += 1
      run = p.add_run('Step '+str(SecMainStepNum)+': Adding Sidesert')
      run.bold = True
      run.font.size = Pt(12)
      SubStepNum = 0
      ...
      if ssparentstep1:
         ...
         if sschildstep1_1:
            ...
            steporder = '\u2206 Step '+str(SecMainStepNum)+'-'+alphabet[SubStepNum]
            run = p.add_run(steporder)
            run.font.bold = True
...
if topsert:
   with st.expander('Select Steps for Sidesert Process',expanded=True):
      ...
      SecMainStepNum += 1
      run = p.add_run('Step '+str(SecMainStepNum)+': Adding Topsert')
      run.bold = True
      run.font.size = Pt(12)
      SubStepNum = 0
...
if cartoning:
   with st.expander('Select Steps for Cartoning Process',expanded=True):
      ...
      # Main Process Name
      SecMainStepNum += 1
      run = p.add_run('Step '+str(SecMainStepNum)+': Cartoning')
      run.bold = True
      run.font.size = Pt(12)
      SubStepNum = 0
...
if bundling and cartoning:  
   with st.expander('Select Steps for Bundling Process (for cartoned product)',expanded=True):
      ...
      # Main Process Name
      SecMainStepNum += 1
      run = p.add_run('Step '+str(SecMainStepNum)+': Bundling after Cartoning')
      run.bold = True
      run.font.size = Pt(12)
      SubStepNum = 0
      ...
      run = p.add_run("\nNote: Once machine is set up for processing, minor adjustments may be needed.")
      run.font.bold = True
      p = document.add_paragraph(style=document.styles['List Bullet 1'])
      p.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
      SubStepNum += 1
...
if bundling and not cartoning:
    with st.expander('Select Steps for Bundling Process (for bottles)',expanded=True):
      ...
      # Main Process Name
      SecMainStepNum += 1
      run = p.add_run('Step '+str(SecMainStepNum)+': Bundling for bottles')
      run.bold = True
      run.font.size = Pt(12)
      #bundlingformat = st.text_input("Please, provide bundling format (ex. 3 cartons in a row = 3 x 1 x 1 [L x W x H])")
#----------------------
